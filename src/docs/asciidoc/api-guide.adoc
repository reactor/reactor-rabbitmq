...
The `Sender` uses 2 Reactor's `Scheduler`s: one for the subscription when creating the
connection and another one for resources management. The `Sender`s defaults
to 2 elastic scheduler, this can be overriden in the `SenderOptions`. The `Sender`
